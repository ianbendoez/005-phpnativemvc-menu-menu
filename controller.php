...
if($controller == 'delete' && $delete == "y"){
  ...
  $urut_menu = htmlspecialchars($_POST['urut_menu']);
  $icon_menu = htmlspecialchars($_POST['icon_menu']);

  $run = $db->create($nama_menu,$urut_menu,$icon_menu);
  $retval['status'] = $run[0];
  $retval['title'] = $run[1];
  $retval['message'] = $run[2];
  ...
  $run = $db->update($id,$nama_menu,$urut_menu,$icon_menu);
  $retval['status'] = $run[0];
  $retval['title'] = $run[1];
  $retval['message'] = $run[2];
